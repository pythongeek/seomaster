  ...
  priorityMatrix.slice(0, 25).forEach(p => {
    opportunities.push({
      ...
      actionPlan: p.recommendedAction ? {
        actionType: (p.category ?? 'position') as ActionPlan['actionType'],
        priority: (p.impact === 'critical' ? 10 : p.impact === 'high' ? 5 : p.impact === 'medium' ? 3 : 1) as ActionPlan['priority'],
        effort: (p.effort ?? 'medium') as ActionPlan['effort'],
  ...
  quickWins.slice(0, 15).forEach(qw => {
    opportunities.push({
      ...
      actionPlan: qw.action ? {
        actionType: 'quick_win_content' as ActionPlan['actionType'],
        priority: 3 as ActionPlan['priority'],
        effort: (qw.effort ?? 'low') as ActionPlan['effort'],
